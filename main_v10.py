...
def split_dataset(source_folder, train_folder, val_folder, val_ratio):
    ...
    # Define the source images and labels folders
    source_images_folder = os.path.join(source_folder, 'images')
    source_labels_folder = os.path.join(source_folder, 'labels')

    # Check if source folders exist
    if not os.path.exists(source_images_folder) or not os.path.exists(source_labels_folder):
        print("Source folders for images or labels do not exist.")
        return

    # Get all jpg and txt files from the source folder (now adjusted_labels/images and adjusted_labels/labels)
    jpg_files = [f for f in os.listdir(source_images_folder) if f.endswith('.jpg')]
    txt_files = [f for f in os.listdir(source_labels_folder) if f.endswith('.txt')]
    ...
    move_files(val_files_base, source_images_folder, source_labels_folder, val_images_folder, val_labels_folder)

    # Move training files
    move_files(train_files_base, source_images_folder, source_labels_folder, train_images_folder, train_labels_folder)
...
output_folder = 'rotation_output_0.4'
