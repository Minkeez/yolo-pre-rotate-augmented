...
def augment_images_and_labels(image_folder, label_folder, output_folder, desired_ratio):
    ...
    augmented_counts = display_label_count_ratio(os.path.join(output_folder, 'labels'), "After Augmentation")
    ...
    for img_file in os.listdir(os.path.join(output_folder, 'images')):
        if img_file.endswith('.jpg'):
            shutil.copy(os.path.join(output_folder, 'images', img_file), os.path.join(output_folder, 'adjusted_labels', img_file))
    ...
    display_label_count_ratio(os.path.join(output_folder, 'train'), "Training Set")
    display_label_count_ratio(os.path.join(output_folder, 'val'), "Validation Set")
...
def display_label_count_ratio(folder_path, description):
    ...
    print(f"{description} - Total: {total}, Class 0: {total_count_0}, Class 1: {total_count_1}, Ratio 0: {ratio_0:.2f}, Ratio 1: {ratio_1:.2f}")
...
def split_dataset(source_folder, train_folder, val_folder, val_ratio):
    """Split dataset into training and validation sets based on a specified ratio, preserving class distribution."""
    ...
    print(f"Common base files found: {len(common_files_base)}")

    # Check if there are no common files
    ...
    for base_name in common_files_base:
        txt_file_path = os.path.join(source_folder, base_name + '.txt')
    ...
    print(f"Class 0 files: {len(class_0_files)}")
    print(f"Class 1 files: {len(class_1_files)}")

    # Determine the number of validation files needed for each class
    val_size_class_1 = int(len(class_1_files) * val_ratio)
    val_size_class_0 = int(len(class_0_files) * val_ratio)

    print(f"Validation set size - Class 0: {val_size_class_0}, Class 1: {val_size_class_1}")
    ...
    train_files_base = set(common_files_base) - val_files_base

    print(f"Training set size: {len(train_files_base)}, Validation set size: {len(val_files_base)}")
    ...
    def move_files(files_base, src_folder, dest_folder):
        for base_name in files_base:
            ...
            if os.path.exists(jpg_source_path) and os.path.exists(txt_source_path):
                ...
                print(f"Warning: Missing files for base {base_name}. JPG: {jpg_source_path}, TXT: {txt_source_path}")
